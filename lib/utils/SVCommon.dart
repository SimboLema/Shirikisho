...
import '../screen/home/addPost/components/CreateGroupBottomSheetComponent.dart';
import '../screen/home/addPost/components/SVSharePostBottomSheetComponent.dart';
import 'SVColors.dart';
...
void ShowCreateGroupBottomSheet(BuildContext context,officers) {
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    enableDrag: true,
    ...
    shape: RoundedRectangleBorder(borderRadius: radiusOnly(topLeft: 30, topRight: 30)),
    builder: (context) {
      return CreateGroupBottomSheetComponent(officers);
    },
  );
}
...
Widget svCommonCachedNetworkImage(
  ...
  if (url!.validate().isEmpty) {
    ...
    return svCommonCachedNetworkImage('${Environment.imageUrl}/$url', height: height, width: width, fit: fit, alignment: alignment ?? Alignment.center).cornerRadiusWithClipRRect(radius ?? defaultRadius);
  }
}

Widget placeHolderWidget({double? height, double? width, BoxFit? fit, AlignmentGeometry? alignment, double? radius}) {
  return Image.asset('images/walletApp/placeholder.jpg', height: height, width: width, fit: fit ?? BoxFit.cover, alignment: alignment ?? Alignment.center).cornerRadiusWithClipRRect(radius ?? defaultRadius);
}
